...
## 구조 선택 기준 — BC별 4레이어 완전분리 vs Martin Joo 2단 구조

두 구조 모두 유효하지만, 프로젝트 상황에 따라 적합도가 갈린다. [[Design Philosophy]] 원칙 2(패턴은 가이드라인)에 따라 아래 축으로 판단한다.

| 판단 축 | BC별 4레이어 (Domain/Application/Infrastructure) | Martin Joo 2단 (Domain/App) |
|---|---|---|
| **Bounded Context 개수** | 실제로 여러 개(Order, Catalog, Identity처럼 언어·모델이 서로 다름) | 사실상 단일 도메인 — 폴더만 기능별로 나뉘는 수준 |
| **애그리거트/불변식 복잡도** | 상태 전이·트랜잭션 일관성이 핵심(주문 확정, 재고 차감 등) | 참조 데이터 매칭·계산 중심 — [[Design Philosophy]]의 "얕은 DDD" 사례(LCA API)처럼 상태 전이형 애그리거트가 거의 없는 경우 |
| **Eloquent 분리 필요도** | Repository + Mapper로 완전 격리 필요(순수 단위 테스트, 인프라 교체 가능성) | [[Eloquent and DDD]] 전략 1(Eloquent = 도메인 모델)로 충분 |
| **팀 구조** | 컨텍스트 경계가 협업 경계와 일치(팀별로 컨텍스트 소유) | 소수 팀이 코드베이스 전체를 넘나듦 — 기술 계층보다 도메인 폴더 탐색 편의가 더 중요 |
| **인터페이스 다양성** | 단일 UI | Web(Inertia)/API/Console처럼 같은 도메인을 여러 "애플리케이션"이 소비 |

애매하면 가벼운 쪽(2단)에서 시작하고, 실제로 컨텍스트 간 언어 충돌이나 트랜잭션 일관성 경계 문제가 드러날 때 BC별 분리로 옮겨간다 — 나중에 쪼개는 것이 처음부터 무겁게 시작하는 것보다 되돌리기 쉽다.
